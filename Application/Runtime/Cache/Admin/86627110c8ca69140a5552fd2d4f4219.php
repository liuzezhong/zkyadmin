                ...
                <div class="col-xs-12">
                    <div class="panel panel-default">
                        <div class="panel-body panel-body-footer">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    <select class="form-control select2" style="width: 100%;" name="match_id">
                                        ...
                                        <?php if(is_array($matchs)): $i = 0; $__LIST__ = $matchs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$match): $mod = ($i % 2 );++$i; if($match["selected"] == 1): ?><option value="<?php echo ($match["match_id"]); ?>" selected="selected"><?php echo ($match["match_title"]); ?></option>
                                                <?php else: ?>
                                                <option value="<?php echo ($match["match_id"]); ?>"><?php echo ($match["match_title"]); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; ?>
                                    </select>
                                </div>
                                <!-- /.form-group -->
                            </div>
                            ...
                            <?php if($enrols != is_null()): ?><div class="col-md-2">
                                    <button type="button" class="btn btn-default" id="export-enrol" match_id="<?php echo ($this_match["match_id"]); ?>"><i class="fa fa-file-excel-o fa-fw"></i>导出报名信息</button>
                                </div><?php endif; ?>
                ...
                <div class="col-xs-12">
                    <div class="box">
                        <div class="box-header">
                            <h3 class="box-title">赛事名称：<?php echo ($this_match["match_title"]); ?></h3>
                        </div>
                        <!-- /.box-header -->
                        <div class="box-body">
                            <table id="example2" class="table table-bordered table-hover">
                                ...
                                <?php if(is_array($enrols)): $i = 0; $__LIST__ = $enrols;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$enrol): $mod = ($i % 2 );++$i;?><tr>
                                        <td class="text-center"><?php echo ($enrol["category_name"]); ?></td>
                                        <td class="text-center"><?php echo ($enrol["real_name"]); ?></td>
                                        <td class="text-center">
                                            <?php if($enrol["real_sex"] == 0): ?>未知
                                                <?php elseif($enrol["real_sex"] == 1): ?>
                                                男
                                                <?php elseif($enrol["real_sex"] == 2): ?>
                                                女<?php endif; ?>
                                        </td>
                                        <td class="text-center"><?php echo ($enrol["id_card"]); ?></td>
                                        <td class="text-center"><?php echo ($enrol["phone_number"]); ?></td>
                                        <td class="text-center">
                                            <?php if($enrol["team_name"] != is_null()): echo ($enrol["team_name"]); ?>队
                                                <?php else: ?>
                                                个人参赛<?php endif; ?>

                                        </td>
                                        <td class="text-center"><?php echo ($enrol["money"]); ?>元</td>
                                        <td class="text-center"><button type="button" class="btn btn-default btn-xs">查看</button></td>
                                        <td class="text-center"><a type="button" class="btn btn-default btn-xs" href="<?php echo ($enrol["sign_qrcode"]); ?>" target="_blank">查看</a></td>
                                        <td class="text-center">
                                            <?php if($enrol["gmt_sign"] != is_null()): echo ($enrol["gmt_sign"]); ?>
                                                <?php else: ?>
                                                <text class="text-danger">未签到</text><?php endif; ?>
                                        </td>
                                        <td class="text-center"><?php echo ($enrol["gmt_create"]); ?></td>
                                        <!--<td class="text-center">-->
                                            <!--<?php if($enrol["is_finish"] == 0): ?>-->
                                                <!--<span class="label label-warning" id="change-status" attr-id="<?php echo ($match["match_id"]); ?>" attr-status="<?php echo ($match["is_finish"]); ?>" attr-m="admin" attr-c="match" attr-a="changeStatus">进行中</span>-->
                                                <!--<?php elseif($match["is_finish"] == 1): ?>-->
                                                <!--<span class="label label-success" id="change-status" attr-id="<?php echo ($match["match_id"]); ?>" attr-status="<?php echo ($match["is_finish"]); ?>" attr-m="admin" attr-c="match" attr-a="changeStatus">已结束</span>-->
                                                <!--<?php elseif($match["is_finish"] == 2): ?>-->
                                                <!--<span class="label label-danger">冻结中</span>-->
                                            <!--<?php endif; ?>-->
                                        <!--</td>-->
                                        <td class="text-center">
                                            <span id="customer-edit" customer-id="<?php echo ($enrol["match_id"]); ?>" data-toggle="modal" data-target="#editCustomerModal">
                                                <i class="fa fa-edit fa-fw"></i>
                                            </span>
                                            <?php if($enrol["team_name"] != is_null()): ?><span id="delete-info" attr-id="<?php echo ($enrol["register_id"]); ?>" attr-name="<?php echo ($enrol["team_name"]); ?>队" attr-m="admin" attr-c="enrol" attr-a="delete">
                                                <i class="fa fa-trash-o fa-fw"></i>
                                            </span>

                                                <?php else: ?>
                                                <span id="delete-info" attr-id="<?php echo ($enrol["register_id"]); ?>" attr-name="<?php echo ($enrol["real_name"]); ?>" attr-m="admin" attr-c="enrol" attr-a="delete">
                                                <i class="fa fa-trash-o fa-fw"></i>
                                            </span><?php endif; ?>
